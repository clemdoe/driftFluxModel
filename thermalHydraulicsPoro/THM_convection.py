...
class DFMclass():
    ...
    def createSystemVelocityPressure(self):
        ...
        for i in range(1, 2*self.nCells-1):
            ...
            #Inside the pressure submatrix
                ...
                ai = - areaMatrix[i],
                bi = areaMatrix[i+1],
                di = - ((rho_old[i+1]- rho_old[i])* self.g * self.DV * ((self.poro[i%self.nCells]+ self.poro[(i+1)%self.nCells])/2) / 2) + DI)
    ...
    def createSystemEnthalpy(self):
        ...
        DM1 = self.q__[i] * self.DV * (self.poro[i]) + DI + DI2
    ...
    def createSystemEnthalpyTransient(self):
        ...
        for i in range(1,self.nCells -1):
            ...
            DI = (1/2) * (P_old[i]*areaMatrix[i] - P_old[i-1]*areaMatrix[i-1]) * ((U_old[i]+ ((epsilon_old[i] * (rho_l_old[i] - rho_g_old[i]) * V_gj_old[i])/ rho_old[i]))+ (U_old[i-1]+ ((epsilon_old[i-1] * (rho_l_old[i-1] - rho_g_old[i-1]) * V_gj_old[i-1])/ rho_old[i-1]) ) )
            DI2 = - (epsilon_old[i]*rho_l_old[i]*rho_g_old[i]*Dhfg[i]*V_gj_old[i]*areaMatrix[i]/rho_old[i]) + (epsilon_old[i-1]*rho_l_old[i-1]*rho_g_old[i-1]*Dhfg[i-1]*V_gj_old[i-1]*areaMatrix[i-1]/rho_old[i-1])
            DT1 = -(self.pressureList[self.timeCount][i%self.nCells] * self.areaMatrixList[self.timeCount][i%self.nCells] - P_old[i] * areaMatrix[i])*(self.dx/self.dt) + (self.rhoList[self.timeCount][i%self.nCells] * self.enthalpyList[self.timeCount][i%self.nCells] * areaMatrix[i] * (self.dx / self.dt))
    ...
    def createSystemVelocityPressureTransient(self):
        ...
        print(f'rho_old: {rho_old}')
        print(f'rho_g_old: {rho_g_old}')
        print(f'rho_l_old: {rho_l_old}')
        print(f'areaMatrix: {areaMatrix}')
        print(f'areaMatrix_old_1: {areaMatrix_old_1}')
        print(f'areaMatrix_old_2: {areaMatrix_old_2}')
        print(f'Dhfg: {Dhfg}')
        print(f'x_th_old: {x_th_old}')
        print(f'f: {f}')
        ...
        for i in range(1, 2*self.nCells-1):
            ...
            #Inside the pressure submatrix
                ...
                ai = - areaMatrix[i],
                bi = areaMatrix[i+1],
                di = - ((rho_old[i+1]- rho_old[i])* self.g * self.DV * ((self.poro[i%self.nCells]+ self.poro[(i+1)%self.nCells])/2) / 2) + DI + (self.rhoList[self.timeCount][i%self.nCells] * areaMatrix[i] * self.velocityList[self.timeCount][i%self.nCells] * (self.dx / self.dt)))
