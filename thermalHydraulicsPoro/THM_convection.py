...
class DFMclass():
    ...
    """def compute_T_surf(self):
        ...
        self.Pfin = np.zeros(self.nCells)
        self.h_z = np.zeros(self.nCells)
        for i in range(self.nFaces-1):
            self.Pfin[i] = (self.P[-1][i] + self.P[-1][i+1])/2
            self.h_z[i] = (self.H[-1][i] + self.H[-1][i+1])/2
        self.T_surf = np.zeros(self.nCells)
        self.Hc = np.zeros(self.nCells)
        for i in range(self.nCells):
            #print(f'At axial slice = {i}, Pfin = {self.Pfin[i]}, h_z = {self.h_z[i]}')
