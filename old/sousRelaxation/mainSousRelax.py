...
def main():
    ...
    debut_creation_fields = time.time()
    ...
    for j in range(N_iterations):
        ...
        Time = time.time()
        mesure_iteration[0].append(Time)

        print(f"\n Begin iteration number: {j}")
        
        start_resol = time.time()
        ...
        end_resol = time.time()
        compteur_resol += end_resol - start_resol
        ...
        #Time = time.time()
        #mesure_iteration[1].append(Time)
    ...
    fin_iteration = time.time()

    debut_creation_end_fields = time.time()
    T=[]
    Hlsat = []
    Hgsat = []
    for i in range(sizeMesh):
    ...
    for i in range(sizeMesh):
        start = time.time()
        Tt = IAPWS97(P = P[i], x = 0.5).T
        Pp = IAPWS97(T = T[i], x = 0.5).P
        end = time.time()
        compteur_IAPWS += end-start
        Tsat.append(Tt)
        Psat.append(Pp)
    
    end_creation_end_fields = time.time()
    ...
    fin_plotting = time.time()

    fin = time.time()

    print(f'temps execution programme: {fin - debut}')
    #print(mesure_iteration)
    print(f'temps de création fields: {fin_creation_fields - debut_creation_fields}')
    T_iter_tot = 0
    for i in range(len(I)):
        #print(f'temps dans literation {i}: {mesure_iteration[1][i] - mesure_iteration[0][i]}')
        T_iter_tot += mesure_iteration[1][i] - mesure_iteration[0][i]
    print(f'temps dans les iterations: {T_iter_tot}')
    print(f'temps dans iteration sans calcul: {fin_iteration - debut_iteration}')
    print(f'temps de creation des champ finaux avec IPAWS: {end_creation_end_fields - debut_creation_end_fields}')
    print(f'temps de plotting: {fin_plotting - debut_plotting}')
    print(f'temps sans plotting: {debut_plotting - debut}')
    print(f'temps importation des bibliotheques: {fin_importation_bib - debut}')
    print(f'Temps IAPWS97: {compteur_IAPWS}')
    print(f'Temps résolution: {compteur_resol}')
main()
