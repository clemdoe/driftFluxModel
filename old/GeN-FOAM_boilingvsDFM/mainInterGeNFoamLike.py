...
Height = 1.655 #m
Q = 280000000 #volumetric heat generation rate W/m3
...
epsilon = np.array([i*0.0/sizeMesh for i in range(sizeMesh)])
...
for i in range(sizeMesh):
    rho_g_start, rho_l_start, rho_start = getDensity(epsilon[i], P[i]*(10**(-6)), getTemperature(P[i]*(10**(-6)), H[i]*(10**(-3)), x = epsilon[i]))
    rho[i] = rho_start
    rho_g[i] = rho_g_start
    rho_l[i] = rho_l_start
    ...
    Dhfg[i] = getHfg(getTemperature(P[i]*(10**(-6)), H[i]*(10**(-3)), epsilon[i]))
    C0[i] = getC0(rho_g_start, rho_l_start)
    x_th[i] = getThermodynamicQuality(U[i], epsilon[i], rho_l[i], rho_g[i], rho[i], Vgj_prime[i], areaMatrix[i])
    T[i] = getTemperature(P[i]*(10**(-6)), H[i]*(10**(-3)), epsilon[i])
...
for j in range(N_iterations):
    ...
    for i in range(sizeMesh):
        ...
        rho_g[i], rho_l[i], rho[i], epsilon[i], x_th_old[i], C0[i], V_gj[i], Dhfg[i] = get_parameters(P[i]*(10**(-6)), H[i]*(10**(-3)), rho_l[i], rho_g[i], rho[i], epsilon[i], D_h, g, U[i], areaMatrix[i])
    ...
    rho_g_residual = np.linalg.norm(np.array(rho_g) - np.array(rho_g_old))
    rho_l_residual = np.linalg.norm(np.array(rho_l) - np.array(rho_l_old))
    epsilon_residual = np.linalg.norm(np.array(epsilon) - np.array(epsilon_old))
    x_th_residual = np.linalg.norm(np.array(x_th_old) - np.array(x_th))
...
Hsat = []
for i in range(sizeMesh):
    Tsat.append(IAPWS97(P = P[i]*10**(-6), x = 0.5).T)
    Psat.append(IAPWS97(T = T[i], x = 0.5).P)
    Hsat.append(IAPWS97(T = Tsat[i], x = 0.5).h)
