...
class DFMclassTransient():


    def __init__(self, nCells, uInlet, pOutlet, hInlet, height, fuelRadius, cladRadius, waterGap,  numericalMethod, frfaccorel, P2P2corel, voidFractionCorrel, Qmax, Qtype, previousDensity, previousVelocity, previousPressure, previousEnthalpy, timeStep):
    ...
    def resolveMVF(self):
        ...
        DT = (self.Dz/self.dt) * (rho_old[i] * VAR_old[i] * areaMatrix[i] - self.previousDensity[i] * self.previousEnthalpy[i] * areaMatrix[i]) - (self.Dz/self.dt) * ( VAR_old[i - self.nCells] * areaMatrix[i] - self.previousPressure[i] * areaMatrix[i])
        print(f'DT init: {DT}')
        ...
        DM1 = self.q__[i] * self.DV + DI + DI2 + DT
        ...
        for i in range(1, VAR_VFM_Class.N_vol-1):
            ...
            if i < self.nCells-1:
                ...
                bi = 0,
                di =  -(self.previousDensity[i] - rho_old[i])* areaMatrix[i] * self.Dz / self.dt)
            ...
            #Inside the pressure submatrix
                ...
                DT = - (self.Dz/self.dt) * (rho_old[i] * VAR_old[i-self.nCells] * areaMatrix[i] - self.previousDensity[i-self.nCells] * self.previousVelocity[i-self.nCells] * areaMatrix[i-1])
                
                VAR_VFM_Class.set_ADi(self.nCells, 
                ci = 0, 
                ai = - areaMatrix[i],
                bi = areaMatrix[i+1],
                di = - ((rho_old[i+1]- rho_old[i])* self.g * self.DV / 2) + DI + DT)
            ...
            #Inside the enthalpy submatrix
                ...
                DT = (self.Dz/self.dt) * (rho_old[i] * VAR_old[i] * areaMatrix[i] - self.previousDensity[i-2*self.nCells] * self.previousEnthalpy[i-2*self.nCells] * areaMatrix[i]) - (self.Dz/self.dt) * ( VAR_old[i - self.nCells] * areaMatrix[i] - self.previousPressure[i-2*self.nCells] * areaMatrix[i])
                ...
                bi = 0,
                di =  self.q__[i%self.nCells] * self.DV + DI + DI2 + DT)
    ...
    def getPhasesEnthalpy(self, i):
        P = self.P[-1][i]
        #print(f'In getPhasesEnthalpy, P = {P}')
